...
class CD_UDepth(nn.Module):
    ...
    def forward(self, rgb, imt, delta, **kwargs):
        x_rgb = nn.functional.interpolate(self.submodel_rgb(rgb)[-1], [480,640], mode='bilinear', align_corners=True)
        x_imt = nn.functional.interpolate(self.submodel_imt(imt)[-1], [480,640], mode='bilinear', align_corners=True)
